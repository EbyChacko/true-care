...
class Doctor(models.Model):
    personal_details = models.OneToOneField(PersonalDetail,
...
def create_or_update_doctor(sender, instance, created, **kwargs):
    if created or instance.is_doctor:
        ...
        doctor, _ = Doctor.objects.get_or_create(
            personal_details=instance, defaults={'department': department})
...
class Patient(models.Model):
    ...
    personal_details = models.OneToOneField(
        PersonalDetail, null=True, on_delete=models.CASCADE)
...
class booking(models.Model):
    ...
    def __str__(self):
        return (
                f"{self.id}: Booking by {self.patient_id} at "
                f"{self.department} department for {self.doctor}"
            )
...
class DoctorDiagnosis(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.booking.patient_id}'s Diagnosis by "
            f"{self.booking.doctor} on "
            f"{self.booking.booking_date}"
        )
